...
- **PostgreSQL 16/17 with the [`pgvector`](https://github.com/pgvector/pgvector) extension** (the bundled compose file uses `pgvector/pgvector:pg17`). Already running the bundled Postgres on a populated volume? See [Upgrading Postgres to 17](/user-guide/self-hosting/postgres-pg17-migration/) before you pull.
